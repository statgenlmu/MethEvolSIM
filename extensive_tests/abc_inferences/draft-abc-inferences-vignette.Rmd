...
```{r}
# Load data
file_path <- "CGI-Mzebra.txt" # Adjust the path if necessary
...
```{r}
head(cgi_data[cgi_data$chr == "NC_036801.1",])
```




## How to generate the structural distribution .RData file from the reference genome and the CGI annotation
...
filtered_data <- CpG_count[CpG_count$chr == dpp4_chr & CpG_count$start > 2.8e7 & CpG_count$end < 3e7, ]
