...
Note that we average across tips as there can be correlations between the mean sd of tips (e.g. under the case of an event affecting the states at two tips).
...
*Correlation measures how much the state of each site says about the next. So, when there is no variation (e.g. in middle segment state u of a site is followed by state u of the next), there is no correlation. Therefore, avoiding the shores leads to lower correlation values. Maybe we could test with/out borders.*
