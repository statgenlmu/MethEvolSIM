...
compute_meanCor_i(index_islands, minN_CpG = 10, 
                  shore_length = 5, data, sample_n = 1)
...
compute_meanCor_i(index_islands, minN_CpG = 10, 
                  shore_length = 5, data, sample_n = 2)
compute_meanCor_ni(index_nonislands, minN_CpG = 10, 
                   shore_length = 5, data, sample_n = 2)
```
...
For each island structure $i \in \mathcal{I}$ at each cherry tip $t_c \in {t_{c,1}, t_{c,2}$ in each phylogenetic tree cherries, 
...
The null hypothesis \( H_0 \) is that the distributions of methylation states at 
the two tips are the same. The test statistic is calculated using the contingency 
table \( T \) of the methylation state counts for each tip:

$$
H_0: \text{distribution}(t_{c,1}) = \text{distribution}(t_{c,2})
$$

The contingency table \( T_{c,i} \) for tips \( t_{c,1} \) and \( t_{c,2} \) is:
...
of the chi-squared test (i.e., expected counts of at least 5 in each category).

For each cherry $c$ and each island $i$, the methylation data at each cherry tip 
for the specified genomic structures (islands) are compared using the chi-squared test.

$$
p_{\text{value}}(c,i) = \chi^2\text{-test}(T_{c,i})
$$

These p-values are stored in a dataframe for each cherry and for each island.

The significance of the methylation frequency changes is determined based on a 
user-defined threshold \( p_{\text{threshold}} \). If the p-value is smaller than 
the threshold, the change is considered significant. This is done for each cherry 
and each island \( i \in \mathcal{I} \) by comparing each p-value to the threshold:

$$
\text{Significant Change}_{c,i} = 
\begin{cases} 
1, & \text{if } p_{\text{value}}(c,i) < p_{\text{threshold}} \\
...
\( i \in \mathcal{I} \) is computed as the average of the significant changes for each island:
