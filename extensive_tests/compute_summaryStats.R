...
print(paste("data dir", opt[["data-dir"]]))
print(paste("pattern", opt[["pattern"]]))
  RData_files <- list.files(opt[["data-dir"]], pattern = paste0("^", opt[["pattern"]], "_\\d{6}\\.RData$"), full.names = TRUE)



# Set the number of simulations to process
n_sim <- length(RData_files)
## TODO: give n_sim also as argument, check there are as many files as expected
