...
within two different types of genomic structures, which can be interpreted as:
...
Structures are categorized as CpG islands or non-islands (alternativelly archipelagos or non-archipelagos). In the root of the genealogy we
...
most CpG islands with a majority predominantly unmethylated and a small proportion predominantly
methylated, and most non-islands with a majority predominantly methylated and a small
...
and $\beta_{I_m}$) set to small values while ${\alpha_{I_m} < \beta_{I_m}}$ to favour
...
Following the same rationale as for island parameterization, all the regions not belonging
to an island sample a common equilibrium probability triple from two Beta distributions
...
You can simulate DNA methylation along a tree given some data at root, or by simulating
data at root given a structural distribution and letting it evolve.
...
As SSEc transition probabilities are dependent on adjacent sites we need to
approximate SSE based evolution by short time steps, allowing for CpG sites
...
independent and correlated processes. The proportion of each of the two SSE types
is determined by the parameter $\iota$.
...
Iota must be between 0 (non-included) and 1, that is in (0,1].
...
value "U" for CpG islands and "M" for non-CpG islands.
...
2 - Determined by the user providing in `infoStr` additional columns: `u_eqFreq` for $\pi_u$, `p_eqFreq` for $\pi_p$ and `m_eqFreq` for $\pi_m$. Note that when given, each of the structures frequencies needs to sum up to 1, so that for each row $i$, $\pi_u(i)+\pi_p(i)+\pi_m(i)=1$.
...
To get the parameter values of a `combiStructureGenerator` object, it can alternativelly be given as argument to the function `get_parameterValues()`.
