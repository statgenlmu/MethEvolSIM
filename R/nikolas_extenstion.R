...
#get_SrMatrix <- 
singleStructureGenerator$set("public","get_SrMatrix",function(old_eqFreqs,new_eqFreqs, testing=FALSE){
...
singleStructureGenerator$set("public","RE_within",function(Y_seq,Y_eqFreqs,testing = FALSE){
  ...
  if (identical(X_eqFreqs, Y_eqFreqs)) {
  ...
  else if (chosen_eqFreqs == "FreqsX" | chosen_eqFreqs == "FreqsY"){
    ...
    if (chosen_eqFreqs == "FreqsX"){
      ...
      Sr <- self$get_SrMatrix(old_eqFreqs = Y_eqFreqs, new_eqFreqs=X_eqFreqs)
      ...
      for(i in 1:length(new_Y_seq)) {
        new_Y_seq[i] <- sample(1:3, size=1, prob=as.vector(Sr[Y_seq[i],]))
      }
      ...
      eqFreqsChange = T
      #compute the transition matrix for equilibrium frequencies
      Sr <- self$get_SrMatrix(old_eqFreqs = X_eqFreqs, new_eqFreqs=Y_eqFreqs)
      # Change data equilibrium frequencies
      ...
      for(i in 1:length(new_X_seq)) {
        new_X_seq[i] <- sample(1:3, size=1, prob=as.vector(Sr[X_seq[i],]))
      }
