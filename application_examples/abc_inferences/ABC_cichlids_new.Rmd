...
## Params as Exp Value and SD. Log scale

```{r}
transf <- c("log", "none", "log", rep("none", 9))
names(transf) <- names(params_expSD)
transf
```
...
```{r}
plot(expIWE_perIsland, merged_sumStats$cherryFreqsChange_i,
     main = "cherryFreqsChange")
```

```{r}
plot(expIWE_perIsland, merged_sumStats$treeFreqsChange_i,
     main = "cherryFreqsChange")
```
...
```{r}
hist(params$TMRCA_smallestCherry)
```

```{r}
summary(params$TMRCA_smallestCherry)
```


```{r}
plot(params$TMRCA_smallestCherry,
     merged_sumStats$MeanSiteFChange_i)
```

```{r}
plot(params$TMRCA_smallestCherry,
     merged_sumStats$MeanSiteFChange_ni)
```

```{r}
estim <- loclinear_cv$estim$tol0.01[,"TMRCA_smallestCherry"]
param <- loclinear_cv$true[,"TMRCA_smallestCherry"]
(1/100)*sum(abs(estim-param)/param)
```
```{r}
estim <- loclinear_cv$estim$tol0.01[,"mu"]
param <- loclinear_cv$true[,"mu"]
(1/100)*sum(abs(estim-param)/param)
```


## New prior to try for alpha_Ri
```{r}
load("design.RData")
hist(sampled_params$alpha_Ri) # old

```
```{r}
hist(rexp(10000, 0.1), 50) #new
```
